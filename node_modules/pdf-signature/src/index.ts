...
import PDFToHTML = require("pdftohtmljs");
import pdf = require('html-pdf');

class signaturePDF {

    public htmlSignature(htmlPath: string, htmlString: string, x: number, y: number, signature: string, download: boolean, documentName: string, docType: string) {
        /*
        ...
        * and then return the HTML string which is signed.
        ...
        if (htmlPath && htmlString) throw new Error('First or second parameters it should be false');
        let searchEndOfDOM = htmlString.search("</body>");
        if (htmlString) {
            if (!signature && typeof signature === 'undefined') throw new Error('Please add the signature');
            htmlPath = undefined;
            if (searchEndOfDOM !== -1) {
                let stampSignaturePosition = `<div style="position: absolute; left: ${x}px; top: ${y}px;"> <img src="${signature}" alt="signature"/> </div> `;
                let content = htmlString.substr(0, searchEndOfDOM) + stampSignaturePosition + htmlString.substr(searchEndOfDOM);
                if (download && documentName) {
                    let searchHTMLExtension = documentName.search(".html");
                    if (searchHTMLExtension === -1) documentName = documentName + '.html';
                    fs.appendFile(documentName, content, (error: any) => {
        ...
        if (htmlPath && signature) {
            ...
            let allChar = /(.*)/,
                htmlExtension = /(.*).html/,
                checkIfExitExtension = htmlPath.match(htmlExtension),
                notCorrectPath = htmlPath.match(allChar);
            if (notCorrectPath || checkIfExitExtension) {
                htmlPath = `/${htmlPath}.html`;
                htmlPath = htmlPath.replace(".html", "");
            }
            const readContent = (callback: any) => {
                fs.readFile(path.dirname(require.main.filename) + htmlPath, 'utf8', (error, htmlPath) => {
                    if (error) return callback(error);
                    let searchEndOfDOM = htmlPath.search("</body>"),
                        HTMLToString = htmlPath.toString();
                    if (searchEndOfDOM !== -1) {
                        let stampSignaturePosition = `<div style="position: absolute; left: ${x}px; top: ${y}px;"> <img src="${signature}" alt="signature"/> </div> `;
            ...
            readContent((error: any, content: any) => {
                if (download && documentName) {
                    let searchHTMLExtension = documentName.search(".html");
                    if (searchHTMLExtension === -1) documentName = documentName + '.html';
                    fs.appendFile(documentName, content, (error: any) => {
                        ...
                        if (docType === 'pdf' || docType === 'PDF') {
                            const html = fs.readFileSync(path.dirname(require.main.filename) + `/${documentName}`, 'utf8'),
                            ...
                            let pdfDocName = documentName.replace(".html", ".pdf"),
                                outputPath = 'output.html';
                            pdf.create(html, options).toFile(pdfDocName, (err, res) => {
                                if (err) throw new Error(err);
                                fs.unlink(path.dirname(require.main.filename) + `/${outputPath}`, (error) => {
                                    if (error) return;
                                });
                                fs.unlink(path.dirname(require.main.filename) + `/${documentName}`, (error) => {
                                    if (error) return;
                                });
                                return res;
                            });
    ...
    public pdfSignature(pdfPath: string, x: number, y: number, signature: string, download: boolean, documentName: string) {
        /*
        ...
        if (pdfPath && signature) {
            ...
            let allChar = /(.*)/,
                htmlExtension = /(.*).pdf/,
                checkIfExitExtension = pdfPath.match(htmlExtension),
                notCorrectPath = pdfPath.match(allChar);
            if (notCorrectPath || checkIfExitExtension) {
                pdfPath = `/${pdfPath}.pdf`;
                pdfPath = pdfPath.replace(".pdf", ".pdf");
            }
            let converter = new PDFToHTML(path.dirname(require.main.filename) + pdfPath, "/output.html");
            // convert the HTML document to PDF
            converter.convert('default').then(() => {
